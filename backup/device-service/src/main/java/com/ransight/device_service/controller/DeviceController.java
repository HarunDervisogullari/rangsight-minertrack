...
public ResponseEntity<?> updateGalleryNameInDevices(@RequestBody Map<String, String> request) {
    ...
    public void deleteDevice(@PathVariable Long id) {
        ...
        if ("Hotspot".equalsIgnoreCase(device.getType())) {
            ...
            for (Hotspot hotspot : hotspots) {
                hotspotGalleryRepository.deleteByHotspotId(hotspot.getId());
                deviceHotspotRepository.deleteByDeviceIdAndHotspotId(device.getId(), hotspot.getId());
...
private EntityManager entityManager;
    ...
    @Transactional
    public ResponseEntity<String> forceDeleteDevice(@PathVariable Long id) {
        try {
            // Use native SQL to avoid Spring transaction issues
            entityManager.createNativeQuery("DELETE FROM device_hotspot WHERE device_id = ?")
                    .setParameter(1, id)
                    .executeUpdate();
                    
            entityManager.createNativeQuery("DELETE FROM devices WHERE id = ?")
                    .setParameter(1, id)
                    .executeUpdate();
                    
            return ResponseEntity.ok("Device force deleted successfully");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error force deleting device: " + e.getMessage());
        }
    }
}
